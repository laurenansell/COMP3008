...
\textcolor{blue}{The top 15 words from each text do not help us identify the genre. For text 2 the word abernathy could represent the name of a main character or a place of importance in the text. The word abandoned appears frequently in both texts which could indicate that the genres are closely related for these two texts.}

\textcolor{blue}{The bar plot for text shows no words only letter and is therefore not useful at all. We may wish to preform some additional cleaning by removing words of our own choosing.}

```{r,message=FALSE,warning=FALSE}

## Remove all the numbers from text 3


book_3$V1 <- removeNumbers(book_3$V1)

book_3_words<-book_3 %>% select(V1) %>%  unnest_tokens(word, V1)

book_3_words_clean<-book_3_words %>% anti_join(stop_words)

## Define your stopwords, you can either define different words for each text separately or all in one go

my_stop_words<-data.frame(word = c("aber","abernathy","abernathy’s"))
...
book_3_words_clean %>% group_by(word) %>% count() %>% head(15) %>% 
  ggplot( aes(x = word, y = n)) +
  geom_col(fill = "cyan", color = "blue") +
  coord_flip() +
  labs(x = "Unique words",
...
\textcolor{blue}{After some additional cleaning, we still don't have enough information here to make the classification.}
...
wordcloud2(book_2_text_wordcloud,color = "random-dark",shape="heart")
